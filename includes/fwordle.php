...
function fwordleFinalizeWords(PDO $pdo, string $date): void
{
    ...
    for ($pos = 0; $pos < $numBoards; $pos++) {
        ...
        if ($uid !== null) {
            ...
            $chosen     = $choiceRow ? strtolower($choiceRow['word']) : null;
...
function fwordleState(PDO $pdo, string $date, int $userId): array
{
    ...
    $boardHints = []; // position => hint string (or null)
    ...
    // Build board_hints as a sequential array (null where no hint was written).
    $boardHintsArr = [];
    for ($i = 0; $i < $numBoards; $i++) {
        $boardHintsArr[] = $boardHints[$i] ?? null;
    }
