...
`includes/header.php` renders a circular **avatar button** (`#nav-avatar`) next to the logout link, showing `users.avatar` (a base64 image data URL) or the username's initial as a fallback. Clicking it opens an opaque modal with two tabs, **Photo** and **Hats**. Photo: the chosen image is **center-cropped to a square and resized to 256px client-side via canvas** (JPEG), then POSTed to `api/upload-avatar.php` (validates the data URL, caps decoded size, confirms it's a real image with `getimagesizefromstring`). The avatar is stored in the DB (not a filesystem upload dir) so it works across all deployments. `study-counter.php` reads every user's avatar into `USER_AVATARS` for the podium. `api/patch-profile.php` lets a user change their username and/or password (requires `current_password`).

### Cosmetics inventory (hats)

Every user owns one row per registered cosmetic in `user_cosmetics` (item registry + placement bounds in `includes/cosmetics.php`; assets are the PNGs in `assets/images/`). `ensureCosmeticsGranted()` lazily `INSERT IGNORE`s any missing rows for a user (called from `api/get-cosmetics.php`), so a newly added asset just needs a registry entry — existing users pick it up next time they open the Hats tab. At most one item can be `equipped` per user. `api/get-cosmetics.php` (GET) returns the caller's full owned list with placement; `api/patch-cosmetic.php` (POST, `item_key` + optional `equip`/`offset_x`/`offset_y`/`rotation`/`scale`/`flip_x`) equips/unequips and updates placement, clamped server-side to `COSMETIC_BOUNDS`.

The **Hats** tab in the avatar modal (`includes/header.php`) is a live positioning tool: a preview avatar circle with the selected hat overlaid via CSS custom properties (`--hx`/`--hy`/`--hr`/`--hs`/`--hflip`), nudge buttons (up/down/left/right), rotate and size sliders, and a flip button — all update the preview instantly client-side; **Save position** persists via `patch-cosmetic.php`. A grid below lists all owned hats to switch/equip; **Remove hat** unequips.

On `study-counter.php`, `getEquippedHats($pdo)` (in `includes/cosmetics.php`) builds `EQUIPPED_HATS` (username -> hat + placement), server-embedded like `USER_AVATARS`. The podium's `hatImg(username)` renders whichever hat that user has equipped (previously a hardcoded wizard-hat-at-100-hours milestone — that threshold is gone; the hat is now purely inventory-driven) with the same `--hx/--hy/--hr/--hs/--hflip` custom properties as the header preview, so a user's saved placement renders identically in both places. Hats only render on the unfiltered podium (Period = Overall, View = all); weekly/daily, per-module and library podiums stay hat-free. `.sc-hat` (podium) and `.hat-preview-hat` (header) share the same percentage-based geometry so they stay visually in sync at any avatar size.
