...
## Migration to Next.js (in progress)

This repo is mid-migration from the PHP app documented below to a TypeScript/Next.js rewrite that lives entirely in `internship-leaderboard-next/` (own `package.json`, own `CLAUDE.md`/`AGENTS.md`). See `plan.md` at the repo root for the phase-by-phase plan, the stack choice (Next.js 16 App Router, Prisma 7 against the *same* MySQL schema documented below, Auth.js v5 Credentials), and per-phase deviation notes.

- **Status:** functionally complete through Phase 6 (fWordle) plus a redirect-only landing page from Phase 7 — every feature described in this file's Architecture section has a working Next.js equivalent. **Not yet deployed**: Vercel + Hostinger-remote-MySQL wiring (plan.md 1.5) is still pending, and the legacy PHP app below remains the live, deployed system.
- **The PHP app is untouched and still authoritative for production.** Nothing outside `internship-leaderboard-next/` has changed except this note, `plan.md`, and `README.md`'s DB-schema migration notes (kept in sync since Prisma's schema is introspected from that same MySQL schema). The PHP files have not been removed and there is currently no plan.md task to remove them until the Next.js app is actually deployed and cut over.
- When working *inside* `internship-leaderboard-next/`, this file's Architecture/API/DB sections are still the correct **behavioral spec** (the Next.js port intentionally mirrors PHP behavior 1:1 unless a phase note in `plan.md` says otherwise) — but file paths, request flow, and framework idioms differ. Concretely: routes are `app/api/<name>/route.ts` (not `api/<name>.php`), pages are Server Components under `app/(app)/<name>/page.tsx`, DB access is via `lib/prisma.ts` (Prisma, not raw PDO), timestamps go through `lib/dates.ts`'s `dbNow()`/`toDbDateStr()` (Europe/Berlin wall-clock encoded as UTC — see that file's comment for why), polled JSON state payloads keep snake_case field names to mirror the original wire format 1:1 even though the surrounding TS is camelCase, there is no SWR (plain `fetch` + `setInterval`/`visibilitychange`/`pageshow`, matching the Study Counter's precedent), and UI is shadcn/ui + Tailwind rather than the PHP app's custom glass/glow CSS.
- Don't assume a PHP file and its Next.js counterpart are byte-identical — check `plan.md`'s per-phase "Notizen" for intentional deviations (e.g. joker-hint cards not pixel-pinned to their board column, `hint` columns added to `fwordle_words`/`fwordle_choices` that this repo's README didn't originally document, etc.) before porting behavior from one to the other.
