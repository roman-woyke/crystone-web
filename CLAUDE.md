...
- The `.podium-controls` are two labelled groups stacked vertically. **Period** is a vertical segmented control (`.seg-control` → `.seg-btn[data-period]`: Overall / This week / Today). **View** is a chip row (`.filter-chip`) where each toggle has its **own** active colour — "Per module" goes violet (crossfades to the per-module mini-podiums), "Library only" goes blue (restricts both podium views to `at_library=1` sessions) — so the two read as separate. Neither view filter affects the chart.
