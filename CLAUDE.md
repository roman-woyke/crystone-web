...
| `api/study-timer.php` | POST | Single state machine for `study_status` (`action` = start/presence/pause/resume/set_module/reset/stop/log/library); `presence` is a module-less "I'm studying" stopwatch, `pause`/`resume` work on either mode; `set_module` assigns/switches the running session's module (stamping each `study_segments` interval with its module — the first pick stamps the open interval in place, a real switch closes it and opens a fresh one, so the session splits into per-module sub-sessions); `library` toggles the BIB ("at the library") flag without bumping `updated_at` so a paused break timer keeps ticking; `log` takes `modules` (JSON array of module names to keep) and writes **one `study_sessions` row per kept module**, summing that module's live intervals server-side and stamping `at_library`; returns `{me, studying}` (`me.parts` is the per-module live breakdown). `start`/mode=`timer` remain for legacy rows but are no longer triggered by the UI |
...
`study-counter.php` (auth-guarded) lets users log study time two ways: the live, server-backed **"I'm studying"** session in the dock (see *Persistent timer + presence* below) or a **manual** hours/minutes + date entry. The manual form is collapsed behind a **"✍️ Manual logging"** button (`#manual-toggle` reveals `#manual-body`), since live tracking now lives entirely in the dock. The **module list** is the distinct exam titles from `exams` (defaults) plus rows in `study_modules` (custom, flagged with a small "custom" badge). Sessions go to `study_sessions` with `module_name` as a plain string (decoupled from both tables) and a `studied_on` DATE.
...
The study timer is **server-backed** so it survives closing/reloading the page. All state lives in one row per user in `study_status` (helper `includes/study-status.php` builds the JSON payload). A row is a running stopwatch when `started_at IS NOT NULL` and paused ("on break") when it is `NULL`; `elapsed = accumulated + (started_at ? NOW() - started_at : 0)`. The single live flow is driven entirely from the dock's `.studying-actions` buttons (**I'm studying / STOP**, **📘 Module**, **BREAK / Resume**, **📚 BIB**):
- **"I'm studying"** starts a `mode='presence'` row — a module-less stopwatch. (`mode='timer'` and the old `start` action still exist for legacy in-progress rows but are no longer triggered by the UI; the old log-a-session **Timer** tab was removed.)
- **📘 Module** (`set_module`) assigns or switches the running session's module via a modal picker. Each `study_segments` interval is stamped with the module active when it opened: the **first** pick stamps the open interval in place (so pre-pick time isn't orphaned), a **switch** closes the current interval and opens a fresh one — so the session splits into **per-module sub-sessions** that all show separately in the recap (live union now reads `seg.module_name`). The button carries the current module name.
- **STOP** opens the **log checklist** modal built from `me.parts` (the per-module live breakdown): one checkbox row per module (with its summed time), plus a disabled "No module" row for any pre-pick time. **Log selected** sends `modules` (the checked names) to `log`, which writes **one `study_sessions` row per kept module** (summing that module's intervals server-side, attaching its segments); unchecked/unmapped time is discarded. **Discard all** = `stop`.
- `pause`/`resume` ("BREAK" / "Resume") operate on the single row. `api/study-timer.php` is the only endpoint that mutates `study_status`.
