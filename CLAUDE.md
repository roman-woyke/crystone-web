...
- **Keyboard feedback is per-board.** Because one guess hits every board, the on-screen keyboard tints letters for a *single* board at a time; a vertical `1·2·3·4` switcher (left of the keyboard, `kbStatesFor`/`renderKeyboard`) chooses which board's green/orange/grey colours are shown.
