...
- The `.podium-controls` are two labelled groups stacked vertically. **Period** is a vertical **slot-machine wheel** (`#period-wheel` → `.period-item[data-period]`: Overall / This week / Today): a CSS-3D cylinder where the active period spins to the centre slot (click an item or scroll the wheel; `layoutWheel`/`spinPeriod` set each item's `rotateX(...) translateZ(...)`). **View** is a chip row (`.filter-chip`) where each toggle has its **own** active colour — "Per module" goes violet (crossfades to the per-module mini-podiums), "Library only" goes blue (restricts both podium views to `at_library=1` sessions) — so the two read as separate. Neither view filter affects the chart.
