...
| `api/fwordle-hint.php` | POST | Use a joker (`type` = grey/orange/green, `board`) on a board; max 3/day (one per type), one per board, and only reveals info not already known from your guesses (else 422); stores it in `fwordle_hints` and returns the new state |
...
- **Hints / jokers** (`fwordle_hints`, `api/fwordle-hint.php`): 3 per day — one grey (greys 5 absent letters), one orange (a present letter), one green (a letter in its spot) — at most one per board, never on an owned/solved board, and each only reveals info not derivable from the player's own guesses (`fwordleComputeHint` / `fwordleBoardKnowledge`). The UI is a **shared joker bar** above the boards (3 buttons that double as the legend, `renderJokers`): click a joker, then click an eligible board to apply it (`pendingHint` → `.fw-board.pickable`). A used hint renders as an **extra board row beneath its board** (`.fw-hint-rows`, `hintRowHtml`) and tints the keyboard; the win/lose result moved into the status pill row. Opponents show a `🃏` joker count.
- **Board count is set by the previous day's failures**: `fwordleFinalizeWords()` runs when a day arrives. `numBoards = max(1, 4 − failers)`, where a "failer" is a player who used **all** their guesses without solving (`finished = 1 AND solved = 0`); someone who guessed a bit but didn't exhaust their guesses doesn't count, nor do non-players. Each prior-day solver (ordered by `fwordle_results.solved_at`) fills + **owns** one slot with their `fwordle_choices` word (else random backfill); remaining slots are random. Since solvers + failers ≤ players ≤ 4, solver count never exceeds `numBoards`. Edge: 4 play, 1 solves, 3 fail → 1 board, owned by the solver, who is then exempt the next day. Finalize is idempotent and race-safe (re-checks `words_finalized` under `SELECT … FOR UPDATE`).
