...
- The `.podium-controls` are two labelled groups: a **Period** segmented control (`.seg-control` → `.seg-btn[data-period]`: Overall / This week / Today) and a **View** chip row (`.filter-chip`: "Per module" crossfades to the per-module mini-podiums; "Library only" restricts both podium views to `at_library=1` sessions, layering on top of the period — neither affects the chart).
