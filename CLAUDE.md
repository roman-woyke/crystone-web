...
- **Hints / jokers** (`fwordle_hints`, `api/fwordle-hint.php`): 3 per day — one grey (greys 5 absent letters), one orange (a present letter), one green (a letter in its spot) — at most one per board, never on an owned/solved board, and each only reveals info not derivable from the player's own guesses (`fwordleComputeHint` / `fwordleBoardKnowledge`). The UI is a hint row above the boards (one cell per board, columns mirror `.fw-boards`); hints also tint the keyboard, and the win/lose result moved into the status pill row. Opponents show a `🃏` joker count.
